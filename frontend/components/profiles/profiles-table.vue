    ...
    div.table.profiles-table(@scroll="scrollHeaderSideways")
      ...
      div.table-body
        ...
        div.row(v-for="profile in displayedProfiles", :class="{'--selected': selectedProfileIdMap[profile.id]}")
          div.cell(v-for="(label, key) in staticFields", :class="{'status-cell': (key === 'status'), '--rejected': profile.status === 'rejected', '--finished': profile.status === 'finished', '--incoming': profile.status === 'incoming', '--canceled': profile.status === 'canceled'}")
